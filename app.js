(() => {
  ...
  const OVERRIDES_KEY = "parentingKnowledgeXixiOverridesV1";
  const SETTINGS_KEY = "parentingKnowledgeXixiSettingsV1";
  const PROFILE_KEY = "parentingKnowledgeXixiGrowthProfileV1";
  ...
  const pageInfo = {
    ...
    growth: ["成长中心", "区分当前画像、下一阶段与历史成长轨迹"],
  ...
  let activeProfile = loadJson(PROFILE_KEY, window.XIXI_GROWTH_PROFILE || {});
  ...
  function renderView(view, options = {}) {
    ...
    if (view === "growth") renderGrowth(options.section || "current");
  ...
  function renderHome() {
    ...
    root.innerHTML = `
      <div class="growth-hero">
        <span class="eyebrow">熹熹当前阶段 · ${escapeHtml(activeProfile?.stageLabel || "持续更新")}</span>
        <h2>知识不变成旧建议，成长也不会被覆盖。</h2>
        <p>${escapeHtml(activeProfile?.note || "")}</p>
        <div class="hero-actions"><button class="button primary" data-action="view-growth">查看当前画像</button><button class="button secondary" data-action="view-history">成长档案</button></div>
      </div>

      <div class="hero">
        <div class="hero-content">
          <span class="eyebrow">熹熹育儿知识体系 · 本地优先</span>
    ...
    root.querySelector("[data-action='view-growth']").addEventListener("click", () => navigate("growth", { section: "current" }));
    root.querySelector("[data-action='view-history']").addEventListener("click", () => navigate("growth", { section: "history" }));
  ...
  function renderGrowth(initialSection = "current") {
    const root = document.getElementById("view-growth");
    const profile = activeProfile || { currentProfile: [], currentFocus: [], nextStage: [], history: [] };
    const sections = { current: "当前画像", focus: "当前重点", next: "下一阶段准备", history: "成长档案" };
    root.innerHTML = `<div class="growth-hero"><span class="eyebrow">画像版本 ${escapeHtml(profile.profileVersion || "")}</span><h2>${escapeHtml(profile.stageLabel || "熹熹成长档案")}</h2><p>${escapeHtml(profile.note || "")}</p></div><div class="growth-tabs">${Object.entries(sections).map(([id,label])=>`<button class="growth-tab ${id===initialSection?'active':''}" data-growth-section="${id}">${label}</button>`).join("")}</div><div id="growthPanel"></div>`;
    const panel = root.querySelector("#growthPanel");
    const draw = section => {
      root.querySelectorAll("[data-growth-section]").forEach(button => button.classList.toggle("active", button.dataset.growthSection === section));
      if (section === "current") panel.innerHTML = `<div class="card-grid">${profile.currentProfile.map(item=>`<article class="knowledge-card"><span class="status-chip">当前状态</span><h3>${escapeHtml(item.label)}</h3><p>${escapeHtml(item.text)}</p></article>`).join("")}</div>`;
      if (section === "focus") panel.innerHTML = `<div class="focus-list">${profile.currentFocus.map(item=>`<article class="focus-item"><span class="status-chip">${escapeHtml(item.status)}</span><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.text)}</p><div>${(item.knowledgeIds||[]).map(id=>`<button class="link-button" data-article-id="${escapeHtml(id)}">查看相关知识</button>`).join(" ")}</div></article>`).join("")}</div>`;
      if (section === "next") panel.innerHTML = `<div class="card-grid">${profile.nextStage.map(item=>`<article class="knowledge-card"><span class="status-chip">未来2–3个月</span><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.text)}</p></article>`).join("")}</div>`;
      if (section === "history") panel.innerHTML = `<div class="timeline-list">${profile.history.map(item=>`<article class="timeline-item"><div class="timeline-age">${escapeHtml(item.age)}</div><div><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.text)}</p></div></article>`).join("")}</div>`;
      bindArticleButtons(panel);
    };
    root.querySelectorAll("[data-growth-section]").forEach(button => button.addEventListener("click", () => draw(button.dataset.growthSection)));
    draw(initialSection);
  }
  ...
  function renderUpdates() {
    ...
    root.querySelector("#resetButton").addEventListener("click", () => {
      ...
      activeProfile = structuredClone(window.XIXI_GROWTH_PROFILE || {});
      saveJson(PROFILE_KEY, activeProfile);
      toast("已恢复内置知识库");
      renderUpdates();
    });
  }
  ...
  function openArticle(id) {
    ...
    els.articleDialogContent.innerHTML = `
      <article class="article-body">
        ...
        ${(entry.xixi || []).length ? `<h3>熹熹18–19个月阶段记录与当时建议</h3><div class="baby-tips-box"><p class="muted small">以下内容保留为历史阶段资料；是否仍适用，请以“成长中心”的当前画像为准。</p><ul>${entry.xixi.map(item => `<li>${escapeHtml(item)}</li>`).join("")}</ul></div>` : ""}
  ...
  function importPackage(data, source = "文件") {
    ...
    saveJson(OVERRIDES_KEY, overrides);
    if (data.growthProfile && typeof data.growthProfile === "object") {
      activeProfile = structuredClone(data.growthProfile);
      saveJson(PROFILE_KEY, activeProfile);
    }
    activeCategories = buildActiveCategories();
    activeEntries = buildActiveEntries();
