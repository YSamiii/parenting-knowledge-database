(() => {
  ...
  const OVERRIDES_KEY = "parentingKnowledgeOverridesV1";
  const SETTINGS_KEY = "parentingKnowledgeSettingsV1";
  ...
  function renderHome() {
    ...
    const personalizedCount = activeEntries.filter(entry => (entry.xixi || []).length).length;
    ...
    root.innerHTML = `
      ...
      <div class="stats-grid">
        ...
        <div class="stat-card"><div class="stat-number">${personalizedCount}</div><div class="stat-label">含熹熹阶段建议</div></div>
    ...
    root.innerHTML = `
      <div class="ask-layout">
        <div class="panel">
          ...
          <textarea id="questionInput" class="ask-textarea" placeholder="例如：熹熹想拿别人手里的玩具，被制止后一直哭，我应该怎么回应？">${escapeHtml(initialQuestion)}</textarea>
          <div class="example-chips">
            ${[
              "孩子被制止后一直哭怎么办？",
              "入园时告别应该怎么说？",
      ...
      <div class="panel">
        ...
        <div class="form-grid">
          ...
          <div class="field full">
            ...
            <small>关闭后，问答只展示通用知识，不调用条目中的“结合熹熹”建议。</small>
          </div>
        </div>
  ...
  function openArticle(id) {
    ...
    els.articleDialogContent.innerHTML = `
      <article class="article-body">
        ...
        ${settings.personalize && (entry.xixi || []).length ? `<h3>结合${escapeHtml(settings.childName)}</h3><div class="xixi-box"><ul>${entry.xixi.map(item => `<li>${escapeHtml(item.replaceAll("熹熹", settings.childName))}</li>`).join("")}</ul></div>` : ""}
  ...
  function generateAnswerHtml(question) {
    ...
    const personalized = settings.personalize ? entries.flatMap(entry => entry.xixi || []).map(item => item.replaceAll("熹熹", settings.childName)) : [];
